...
class Morpion:
    ...
    def Regle(self, grande_case, joueur):
        ...
        if ((liste[0] == liste[1] == liste[2]) and liste[0] != 0) or (
                (liste[3] == liste[4] == liste[5]) and liste[3] != 0) or (
                (liste[6] == liste[7] == liste[8]) and liste[6] != 0) or (
                (liste[0] == liste[4] == liste[8]) and liste[0] != 0) or (
                (liste[0] == liste[3] == liste[6]) and liste[0] != 0) or (
                (liste[1] == liste[4] == liste[7]) and liste[1] != 0) or (
                (liste[2] == liste[5] == liste[8]) and liste[2] != 0) or (
                (liste[6] == liste[4] == liste[2]) and liste[6] != 0):
    ...
    def Finale(self, joueur):
        if ((self.MegaGrille[0] == self.MegaGrille[1] == self.MegaGrille[2]) and self.MegaGrille[0] != 0) or (
                (self.MegaGrille[3] == self.MegaGrille[4] == self.MegaGrille[5]) and self.MegaGrille[3] != 0) or (
                (self.MegaGrille[6] == self.MegaGrille[7] == self.MegaGrille[8]) and self.MegaGrille[6] != 0) or (
                (self.MegaGrille[0] == self.MegaGrille[4] == self.MegaGrille[8]) and self.MegaGrille[0] != 0) or (
                (self.MegaGrille[0] == self.MegaGrille[3] == self.MegaGrille[6]) and self.MegaGrille[0] != 0) or (
                (self.MegaGrille[1] == self.MegaGrille[4] == self.MegaGrille[7]) and self.MegaGrille[1] != 0) or (
                (self.MegaGrille[2] == self.MegaGrille[5] == self.MegaGrille[8]) and self.MegaGrille[2] != 0) or (
                (self.MegaGrille[6] == self.MegaGrille[4] == self.MegaGrille[2]) and self.MegaGrille[6] != 0):
            self.g.supprimerTout()
    ...
    def Affichage(self, grande_case, petite_case, joueur):
        ...
        if joueur == 1:
            ...
            self.g.placerAuDessous(
                self.g.dessinerRectangle(xc, yc, taille_petite_case, taille_petite_case, "royalblue"))
        if joueur == 2:
    ...
    def gameover(self):  # a changer avec grille
        if ((self.MegaGrille[0] == self.MegaGrille[1] == self.MegaGrille[2]) and self.MegaGrille[0] != 0) or (
                (self.MegaGrille[3] == self.MegaGrille[4] == self.MegaGrille[5]) and self.MegaGrille[3] != 0) or (
                (self.MegaGrille[6] == self.MegaGrille[7] == self.MegaGrille[8]) and self.MegaGrille[6] != 0) or (
                (self.MegaGrille[0] == self.MegaGrille[4] == self.MegaGrille[8]) and self.MegaGrille[0] != 0) or (
    ...
    def minimax(self, grille, depth, is_maximizing):
        """
        ...
        if game_over(self.MegaGrille) or depth == 0:
            return self.evaluate(grille), None  # Évaluer l'état de la grille

        cases = self.actions(grille)  # Obtenir tous les coups possibles

        if is_maximizing:  # Tour de l'IA
            best_value = float('-inf')  # -infini
            best_move = None
            for move in possible_moves:
                new_board = self.apply_move(grille, move,
                                            player=2)  # Simuler coup pour joueur 2, creer fonction qui renvoie une nouvelle grille
                value, _ = minimax(nouvelle_grille, depth - 1, is_maximizing=False)  # Réduire la profondeur
                if value > best_value:
                    best_value = value
                    best_move = move
            ...
            best_move = None
            for move in possible_moves:
                new_board = self.apply_move(grille, move, player=1)  # Simuler coup pour joueur 1
                value, _ = minimax(grille, depth - 1, is_maximizing=True)  # Réduire la profondeur
                if value < best_value:
                    best_value = value
                    best_move = move
            return best_value, best_move

    def actions(self, grille):
        return None  # retourne tt les coups possibles

    def evaluer(self, grille, joueur):
        """
        Évalue l'état actuel du plateau et retourne un score pour le joueur donné.

        on doit chercher et coder d'une facon a ce que le score soit mis a jour d'une bonne facon et mettre
        de l'importance sur les bon moves
        Ici un exemple du chat mais incomplet et pas compatible avec notre code
        """
        for i in range(9):
            for petite_case in self.lists[i]:
                if petite_case == joueur:
                    score += 10
                else:
                    score -= 10

        # 2. Alignements possibles dans la grande grille
        Grille = self.MegaGrille
        for line in big_grid.get_lines():
            player_count = line.count(player)
            opponent_count = line.count(opponent)
            if player_count > 0 and opponent_count == 0:
                score += player_count * 5  # Bonus pour les alignements possibles
            elif opponent_count > 0 and player_count == 0:
                score -= opponent_count * 5  # Pénalité pour alignements adverses

        # 3. Opportunités dans les petites grilles
        for small_grid in board['small_grids']:
            for line in small_grid.get_lines():
                player_count = line.count(player)
                opponent_count = line.count(opponent)
                if player_count > 0 and opponent_count == 0:
        ...
        # 4. Réduction des choix de l'adversaire
        if board['next_grid_restriction'] and board['next_grid_restriction'].is_disadvantageous(opponent):
            score += 3
