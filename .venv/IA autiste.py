...
class Morpion :
    ...
    def IA_golmon(self, cliquable):
        if cliquable is not None:
            grande_case = cliquable
            print("grande case",grande_case)
            liste_cases = self.lists[grande_case]
            petites_cases= []
            for i in range(9):
                if liste_cases[i] == 0:
                    petites_cases.append(i)
            if petites_cases :
                petite_case = random.choice(petites_cases)
                return grande_case, petite_case

        grande_case = []
        for i in range(1,10):
            if i not in self.ban :
                grande_case.append(i)

        if not grande_case:
            return None, None
        Gcase = random.choice(grande_case)
        liste_cases = self.lists[Gcase-1]
        petites_cases = []
        for i in range(9):
            if liste_cases[i] == 0:
                petite_case.append(i)
        petite_case = random.choice(petites_cases)
        return Gcase, petite_case
